...
The `doi-review` critic loads `evidence.md`, `principles.md`, `scope-drift.md`, and `invented-data.md` for this phase. Schema-conformance applies indirectly: the source `scorecard.json` must have already passed the schema-conformance gate in Phase 9 (doi-scorecard). doi-render does not re-validate the JSON — it trusts that gate and refuses to run if `_index.json.scorecard !== "complete"`. The critic's job here is to confirm no content drift (the handoff renders the same verdict, ratings, deployables, and remediation as the source JSON) and no scope drift (the handoff adds nothing not in the source).
